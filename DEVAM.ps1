...
$program = Join-Path $klasor "yalniz_gorsel_devam.py"
$logKlasoru = Join-Path $klasor "logs"
New-Item -ItemType Directory -Force -Path $logKlasoru | Out-Null
$log = Join-Path $logKlasoru ("yalniz_gorsel_devam_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
Start-Transcript -LiteralPath $log -Force | Out-Null

Write-Host "============================================================"
Write-Host "KEP ATLAS YALNIZ GORSEL - KALDIGI YERDEN DEVAM"
Write-Host "============================================================"
...
try {
    ...
    & $python $program --gorsel-isci 3 --gorsel-parti 8 --kaynak-tur 4 `
        --gorsel-hata-deneme 4 --min-genislik 600 --min-yukseklik 400 `
        --kabul-esigi 0.78 --onayla
    $kod = $LASTEXITCODE
    if ($kod -ne 0) { throw "Devam sureci $kod koduyla durdu. Log: $log" }
}
finally {
...
Write-Host "Devam islemi tamamlandi." -ForegroundColor Green
Write-Host "Log: $log"
exit 0
